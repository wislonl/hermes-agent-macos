...
fn malformed_json_returns_parse_error_and_continues() {
    let output = run_runtime(
        r#"not json
{"jsonrpc":"2.0","id":"req_after_parse_error","method":"runtime.handshake","params":{"protocolVersion":"0.1.0"}}"#,
    );
...
fn wrong_json_rpc_version_returns_invalid_request() {
    let output = run_runtime(
        r#"{"jsonrpc":"1.0","id":"bad_version","method":"runtime.handshake","params":{"protocolVersion":"0.1.0"}}"#,
    );
...
fn invalid_envelope_shape_returns_invalid_request() {
    let output = run_runtime(
        r#"{"jsonrpc":"2.0","id":null,"method":"runtime.handshake","params":{"protocolVersion":"0.1.0"}}
